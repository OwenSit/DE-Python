...
def obj_fun(parameters, *data):
    ...
    #     if (result[i]) >= SUM_Q17_THRESHOLD:
    #         result[i] = 1
    #     else:
    #         result[i] = 0
    # assign new data with 1 when >= 0.5, 0 otherwise
    auc = metrics.roc_auc_score(data[:, 7], result)
...
plt.legend(loc=4)
plt.title(f'Plot for {FILENAME} with threshold {Q8_THRESHOLD}')
plt.show()
